...
% Os resultados da avaliação mostram que o seu desempenho supera as abordagens de classificação mais avançadas, tais como um modelo de classificação RoBERT aperfeiçoado. 

Tornamos também disponível publicamente o grafo resultante do processo de extracção, contendo um total de 680 personalidades políticas e referindo mais de 10 mil notícias cobrindo um período de cerca de 25 anos.
\end{resumo}
...
\cite{10.1145/1651461.1651468} aplicaram um método baseado em 

% Propomos e avaliamos um método para a criação automática de um corpus de referência para a formação de textos de classificação para a formação de opiniões políticas mineiras em tenda de con- tos de utilizadores. O processo começa com a compilação de uma colecção de comentários altamente opinativos publicados pelos utilizadores numa notícia em linha - papel. Em seguida, definimos e utilizamos um conjunto de regras de alta precisão elaboradas manualmente e apoiadas por um grande ícone de sentimentos, a fim de identificar frases em cada comentário que expressam opiniões sobre entidades políticas. Finalmente, as opiniões encontradas são propagadas às restantes frases do comentário que mencionam as mesmas entidades, aumentando assim o número e a variedade de frases de opinião. Os resultados mostram que a maioria das regras pode identificar opiniões negativas com muito grande precisão, e estas podem ser propagadas com segurança às restantes frases do comentário em quase 100\% dos casos. Devido a problemas decorrentes da ironia, a precisão da identificação cai para opiniões positivas, mas várias regras ainda atingem uma elevada precisão. A propagação de opiniões positivas é correcta em cerca de 77\% dos casos, e a maioria dos erros nesta fase resulta da inversão da ironia e da polaridade ao longo do comentário.
...
\cite{de-arruda-etal-2015-annotated} criaram um corpus de notícias políticas em português do Brasil, anotando cada parágrafo com o sentimento segundo duas dimensões: o actor político referido pelo parágrafo, e o sentimento dessa referência: positivo, negativo ou neutro. Esta é uma tarefa algo diferente da proposta neste trabalho no sentido em que fica em aberto qual é a origem do sentimento. \cite{BARANIAK20213627} disponibilizam corpora semelhante, anotando o sentimento a uma entidade política em manchetes de jornais on-line, para o Inglês e Polaco.
...
Analisando a frequência de ocorrência de cada entidade observa-se que há pequeno número de entidades responsáveis por uma grande parte de todas as ocorrências de entidades nos dados anotados. Como mostra a Figura~\ref{fig:ent_power_law} existe um número pequeno de entidades frequentes, e uma longa lista de entidades pouco frequentes, em concreto, 96 (19\%) das personalidades são responsáveis por 80\% das menções.

O processo de anotação foi feito por apenas um anotador e sendo que a tarefa de anotação gera alguma ambiguidade muitas das relações foram anotados como \textbf{outra}. 
