...
\author{
  ...
  \email{david.batista@comtravo.com} 
}
...
Os títulos de notícias de política relatam com frequência interacções envolvendo duas ou mais personalidades, sendo que  muitas dessas interacções correspondem a uma relação de apoio ou oposição de uma personalidade para outra. Por exemplo:
...
A análise de um grande número deste tipo de relações ao longo do tempo permite vários estudos, por exemplo, encontrar quais as grandes comunidades de apoio ou oposição em função dos governos no poder ou encontrar as grandes alianças e oposições e as suas dinâmicas. 

Explorando individualmente as relações de uma personalidade ao longo do tempo procurando por padrões onde personalidade emerge como alvo ou origem nas relações. Por exemplo: comparar as relações de apoio ou oposição antes de uma personalidade política tomar posse num determinado cargo público com as relações depois de ter assumido o cargo, ou ver que relações de apoio subitamente emergiram.

Tendo um método automático para extrair estas relações e podendo aplicá-lo a uma colecção de dados abrangendo longos períodos temporais permitiria concretizar os exemplos descritos acima. Também poderia ser usado para rapidamente reunir uma colecção de notícias contendo interacções de apoio ou oposição envolvendo personalidades e partidos políticos específicos, por exemplo para auxiliar numa tarefa de jornalismo de investigação.

Neste trabalho apresentamos um método para extrair relações de apoio ou oposição entre personalidades políticas e descrevemos os resultados da aplicação do mesmo a uma colecção de notícias abrangendo um período de cerca de 25 anos. 

Durante o processo de extração das relações ligamos as personalidades políticas envolvidas ao seu identificador na Wikidata~\citep{MKGGB2018}, enriquecendo assim a relação com informação associada à personalidade (e.g.: afiliação política, cargos públicos exercidos, legislaturas, relações familiares, etc.). Todas as relações extraídas são representadas sob a forma de triplos semânticos seguindo a norma Resource Description Framework (RDF)~\citep{schreiber2014primer}. As personalidades políticas envolvidas, representadas pelo seu identificador na Wikidata, são ligadas através de uma relação de oposição ou apoio representada pela notícia que dá suporte à relação, dando assim origem a um gráfico semântico.

É desta forma possível formular interrogações SPARQL~\citep{2013sparql} envolvendo a informação da Wikidata associada a cada personalidade e as relações extraídas, tais como:

\begin{itemize}
\item{Lista de notícias onde a personalidade X se opõe à personalidade Y}
\item{Todos os membros de um determinado partido que apoiaram alguma personalidade específica}
...
As principais contribuições deste trabalho são: um gráfico semântico RDF ligando personalidades políticas representadas na Wikidata através de uma relação de oposição ou apoio suportada por uma notícia, um conjunto de dados anotados utilizado para treinar os classificadores de extração das relações de títulos de notícias e um interface web que permite explorar o gráfico semântico, permitindo filtrar o tipo de relações, o período de tempo das notícias, e o peso de uma relação, ou seja, o número de noticias que suportam uma relação entre duas personalidades.

Este artigo está organizado da seguinte forma: na Secção~\ref{sec_related_work} referimos trabalho relacionado, na Secção~\ref{sec_kb} descrevemos a Baseada de Conhecimento usado no processo de extracção de relações. A Secção~\ref{sec:data_sources} refere e descreve fontes de notícias utilizadas. Na Secção~\ref{sec:rel_data_annot} detalhamos o conjunto de dados anotados e na Secção~\ref{sec:classifiers} os classificadores desenvolvidos. Na Secção\ref{sec:pipeline} descrevemos o processo extracção de triplos e a construção do gráfico semântico. Finalmente, na Secção~\ref{sec:future_work} apresentamos ideias para melhorar e continuar este trabalho.


\section{Trabalho Relacionado}
\label{sec_related_work}

A análise sentimento no contexto processamento de linguagem natural (PLN) tem sido alvo de estudo maioritariamente em conteúdo gerado em redes redes sociais~\citep{10.1145/3185045} ou na avaliação de produtos ou serviços~\citep{pontiki-etal-2016-semeval}. Nestes domínios o autor da e o alvo da opinião são explícitos.

No entanto este tipo de abordagem não se aplica no contexto de análise de notícias de política, onde existe com frequência um sentimento expresso entre actores políticos sob a forma de relações de apoio ou oposição entre eles~\citep{balahur2009opinion, balahur-etal-2010-sentiment}.
...
Diversos trabalhos exploram relações com sentimento positivo ou negativo entre entidades mencionadas em notícias. \cite{park-etal-2021-blames} define a tarefa de extracção de sentimento direccionada: dada uma frase $s$ referindo duas entidades $p$ e $q$,  detectar qual a relação de sentimentos entre $p$ e $q$ de entre cinco possíveis: neutra, $p$ tem uma opinião positiva ou negativa em relação a $q$, ou $q$ tem uma opinião positiva ou negativa em relação a $p$.
...
\cite{liang2019blames} define a tarefa de extração de relações de culpabilidade: dado um artigo $d$ e um conjunto de entidades $e$, há uma relação de culpabilidade $(s,t)$ quando $s$ culpa $t$ com base no artigo $d$, sendo que há $|e| \cdot (|e| - 1)$ possíveis relações de culpabilidade, .


% https://aclanthology.org/P13-1108/
% https://aclanthology.org/N19-1167/

Alguns autores exploram estas relações num contexto de política internacional entre nações. \cite{oconnor-etal-2013-learning} propõem um modelo não supervisionado baseado em \textit{topic models} extraindo eventos ocorridos num determinado período de tempo envolvendo duas nações, e não apenas relações de conjunto fechado. \cite{han-etal-2019-permanent} geram de forma não supervisionada descritores de relações para pares de nações mencionadas em notícias, e treinam um encoder-decoder model de forma não supervisionada. Dado um par de nações e contexto textual, o modelo gera $d$ \textit{embeddings} associados a um descritor da relação, que são usados para ordenar os descritores candidatos a descrever a relação.
...
Esta tarefa poderá ser vista como uma intersecção entre duas tarefas de PLN, extracção de relações e análise de sentimento.

No contexto de análise de notícias de política em Português, e tanto quanto é do meu conhecimento, julgo este ser um dos primeiros trabalhos a extrair de forma automática de notícias relações de apoio e oposição entre personalidades de forma a poder construir um grafo semântico.
...
\item pessoas portuguesas nascidas depois de 1935 cuja profissão seja: juiz, economista, advogado, funcionário público, político, empresário ou banqueiro
\item pessoas que têm ou tiveram pelo menos um cargo de uma lista de cargos públicos portugueses previamente seleccionados (e.g.: ministério, líder do partido, embaixador, etc.)
\end{itemize}  

Para além dos resultados destas interrogações, seleccionámos manualmente alguns identificadores de personalidades não abrangidos pelas interrogações SPARQL definidas acima. Acrescentamos também todos os identificadores de possíveis partidos políticos a que as personalidades possam estar afiliadas. Este processo resultou num total de {\bf XXX} personalidades e de {\bf XXX} partidos políticos. Descarregamos depois a página na Wikidata para cada um dos identificadores recolhidos, utilizando outro \textit{endpoint}\footnote{\url{https://www.wikidata.org/wiki/Special:EntityData?}}.
...
A principal fonte de notícias foi o arquivo da web portuguesa~\citep{SearchPastPWA2013}. Usando a \textit{API} pública de pesquisa recolhemos notícias restringindo os resultados a ocorrências dos nomes reunidos na Secção~\ref{sec_kb} e a 45 domínios .pt associados a diversas fontes de informação: jornais \textit{on-line}, \textit{websites} de estações de televisão e rádio, e portais agregadores de conteúdos. 
...
Deste processo resultou uma colecção de cerca de 13,7 milhões de artigos publicados entre 1994 e 2019. De seguida foi aplicado um pré-processamento de modo a remover notícias com: títulos duplicados, títulos com menos de 4 palavras, e títulos ou URL contendo palavras de uma lista pré-definida que sugerem um o outro contexto que não política (e.g.: desporto, celebridades, artes, cinema). Este pré-processamento resultou em 1.37 milhões de títulos, cerca de 10\% dos dados inicialmente recolhidos.

A grande maioria das notícias tem como origem o arquivo da web portuguesa, através da sua \textit{API} que retorna o título e o HTML original da página. Limpar os dados, de forma a ter apenas o texto da notícia não é uma tarefa simples, especialmente considerando o variedade de domínios e o período temporal. Por esta razão optamos por focar a extracção de relações no título das notícias.


\section{Colecção de Relações Anotadas}
\label{sec:rel_data_annot}

De forma a poder treinar classificadores de aprendizagem estatística para identificar automáticamente as relações presentes nos títulos das notícias, anotámos manualmente num conjunto de títulos: as personalidades, os identificadores na Wikidata e a relação entre as personalidades.

Começamos por pré-processar todos os títulos automáticamente. Usando o pacote de software spaCy 3.0~\citep{spacy} e o modelo~\texttt{pt\_core\_news\_lg-3.0.0} fizemos o reconhecimento de entidades mencionadas do tipo \texttt{PESSOA}, e para cada tentamos encontrar o seu identificador correspondente na Wikidata fazendo uma interrogação ao índice descrito na Secção~\ref{sec_kb} e assumindo que na lista de resultados o primeiro é o identificador a associar à entidade. Seleccionámos depois apenas títulos referindo duas pessoas e ambas com um identificador Wikidata associado.

Fomos depois seleccionando aleatoriamente e anotando, e corrigindo sempre que necessário as entidades e o identificador na Wikidata, a relação existente: \textbf{oposição} ou \textbf{apoio}, e a direcção. Quando nenhuma das duas se verifica a relação é anotada como \textbf{outra}. A Tabela~\ref{tab:samples} demonstra alguns exemplos das relações anotadas.

Este processo resultou num conjunto de dados contendo {\bf XXXX} títulos anotados e {\bf XXXX} personalidades políticas. A Tabela~\ref{tab:rel_dataset} caracteriza os dados em termos de número de relações e direcção. É de notar que a maioria dos títulos contém uma relação de \textbf{oposição} ou \textbf{outra}, e a grande maioria as relações têm uma direcção da primeira entidade para a segunda, Ent\textsubscript{1}$\rightarrow$Ent\textsubscript{2}.
...
Se nenhum candidato for gerado na primeira fase, o algoritmo tenta expandir as entidades mencionadas no título com base no texto da notícia, explorando o padrão em que uma personalidade mencionada no título por um versão curta do seu nome é também referida no texto da notícia por um nome mais completo.

O algoritmo começa por identificar todas as pessoas mencionadas no texto da notícia, usando a componente descrita na secção~\ref{subsec:ner} e descartando todas que não têm pelo menos um nome em comum com a personalidade a ser desambiguada.  Se o processo de expansão resulta em mais do que uma entidade expandida, escolhemos um candidato apenas se existir um com uma similaridade 1.0 para com a entidade expandida.

Se por outro lado, deste processo resulta apenas uma entidade expandida e se há uma similaridade de 1.0 com apenas um dos candidatos anteriormente seleccionados da BC, esse candidato é escolhido. Caso contrário a entidade expandida é usada para recolher uma nova lista de candidatos da BC, se nessa lista apenas há um candidato e a sua similaridade Jaro é de pelo menos 0.8 com a entidade expandida, esse candidato é escolhido. Se há mais do que um candidatos e apenas um tem uma similaridade 1.0 com a entidade expandida esse é escolhido. Em qualquer outro caso nenhum candidato é retornado.
...
Os resultados desta abordagem sobre o conjunto de dados anotados e descritos na Secção~\ref{sec:rel_data_annot} são descritos na Tabela~\ref{tab:ent_linking_results}. A classificação incorrecta corresponde a personalidades que não foram associadas ao identificador Wikidata incorrecto, e não desambiguada aquelas para as quais o algoritmo não conseguiu chegar a um identificador único.

São relatadas duas avaliações diferentes, Eval. 1 são os resultados para o algoritmo, tal como descrito acima. Eval. 2 é o mesmo algoritmo mas com dois mapeamentos adicionais: \textit{Cavaco} para \textit{Cavaco Silva} e \textit{Marques Mendes} para \textit{Luís Marques Mendes}.
...
Decidimos decompor a tarefa de classificação do tipo de relação, descritas na Tabela~\ref{tab:rel_dataset}, em dois classificadores diferentes: um classificador para o tipo de relação e outro para a direcção da relação, por oposição a treinar um único classificador que teria que distinguir de entre 5 classes.

Esta secção descreve o classificador do tipo de relação presente num título, tendo 3 classes possíveis: \textbf{opõe-se}, \textbf{apoia} e \textbf{outra}. 

%Utilizámos uma abordagem baseada em vectores TF-IDF~\citep{DBLP:journals/ipm/SaltonB88} como base de referência, e alimentámo-los com um classificador linear SVM~\citep{cortes1995support}.

\begin{table}[!h]
    \begin{center}
    \begin{tabular}{l cccr}
        {\bf Relação} & {\bf P} & {\bf A} & {\bf F\textsubscript{1}} & {\bf \#Títulos} \\
        \hline
        opõe-se          &  0.72  &  0.69  &  0.71  & 1099 \\
        outra            &  0.69  &  0.68  &  0.69  &  519 \\
        apoia            &  0.57  &  0.63  &  0.60  & 1064 \\
		\hline
		Macro-Média      &	0.68  &  0.68  &  0.68  & 2682 \\
    \end{tabular}
...
Este classificador tem 2 classes possíveis. O conjunto de dados tem um viés para com a classe Ent\textsubscript{1} $\rightarrow$ Ent\textsubscript{2} representando 91,5\% dos dados. Assim, optamos por desenvolver uma abordagem baseada em regras para detectar apenas a classe Ent\textsubscript{2}$\leftarrow$Ent\textsubscript{1}, e sempre que nenhuma das se verificar o classificador atribui a classe Ent\textsubscript{1} $\rightarrow$ Ent\textsubscript{2}.
...
Os classificadores descritos na secção anterior formam o processo de extracção de triplos RDF a partir dos títulos de notícias recolhidos do arquivo da web portuguesa, formando assim um grafo que pode depois ser interrogado com a linguagem SPARQL.
